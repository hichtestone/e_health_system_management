...
class ListGen
{
    ...
    private function renderTHead(): string
    {
        ...
        foreach ($this->columns as $column) {
            ...
            if ($column->isSortable()) {
                $html .= ' <a class="lg-sort" href="'.$this->getAjaxUrl().'"><i class="fa fa-sort'.('ASC' == $this->sorts[$i]->getOrder() ? '-down' : ('DESC' == $this->sorts[$i]->getOrder() ? '-up' : '')).'"></i><sup></sup></a>';
            }
            $html .= '</th>';
            ++$i;
        }
    ...
    private function renderToolBarTop(): string
    {
        $html = $this->renderHtmlElement('form', [
