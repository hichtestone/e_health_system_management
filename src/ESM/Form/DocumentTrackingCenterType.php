...
/**
 * Class DocumentTrackingCenterType
 * @package App\ESM\Form
 */
class DocumentTrackingCenterType extends AbstractType
{
    /**
    ...
    {
        $centerCountries = [];
        foreach ($options['center']->getInstitutions() as $institution) {
            if (!in_array($institution->getCountry(), $centerCountries)) {
                $centerCountries[] = $institution->getCountry();
            }
        }

        $alreadyDoc = $this->em->getRepository(DocumentTrackingCenter::class)
            ->findBy(['center' => $options['center']]);
    ...
    public function configureOptions(OptionsResolver $resolver)
    {
        $resolver->setDefaults([
            'data_class' => DocumentTrackingCenter::class,
        ]);
