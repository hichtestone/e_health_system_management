...
/**
 * filtre basé sur une date nullable en select (oui/non/tous) (exemple: isObject).
 */
class FilterBool extends AbstractFilterType
{
    /**
    ...
    public function render(string $label, ?TranslatorInterface $translator): string
    {
        ...
        $html .= '<option value="0"'.('0' == $value ? ' selected="selected"' : '').'>'.$translator->trans('word.text.no').'</option>';
        $html .= '<option value="1"'.('1' == $value ? ' selected="selected"' : '').'>'.$translator->trans('word.text.yes').'</option>';
        $html .= '<option value="2"'.('2' == $value ? ' selected="selected"' : '').'>'.$translator->trans('word.text.all').'</option>';
        $html .= '</select>';
