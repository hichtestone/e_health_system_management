...
namespace App\ESM\Service\Mailer;
...
class MailerService
{
	 ...
	 * @throws SyntaxError
	 */
	public function sendEmailETMF(string $template, array $template_params, string $subject, array $subject_params, array $to, array $cc = [], array $bcc = []): void
	{
		$subjectPrefix = '[ETMF Unicancer]';
		$bcc[] = self::$EMAIL_DEV;
		$bcc[] = self::$EMAIL_ADMIN;
		...
		$mail->setFrom(self::$EMAIL_FROM, self::$FROM_NAME)
			...
			->setReturnPath(self::$EMAIL_FROM)
			->setSubject($subjectPrefix.$this->translator->trans($subject, $subject_params))
			->setBody($this->renderer->render('ETMF/email/' . $template . '.html.twig', $template_params), 'text/html')
			->addPart($this->renderer->render('ETMF/email/' . $template . '.txt.twig', $template_params), 'text/plain')
		;
